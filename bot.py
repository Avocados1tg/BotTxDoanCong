...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'play':
        ...
        await context.bot.send_message(chat_id=query.message.chat_id, text='💰 **Chọn mức cược:**\n*(Càng lớn càng kịch tính!)* 🎰', parse_mode='Markdown', reply_markup=reply_markup)
        return
        ...
        if balance < bet:
            ...
            await context.bot.send_message(chat_id=query.message.chat_id, text=f'❌ **Hết tiền rồi!** 😱\nCòn *{balance} điểm*. Cược nhỏ hơn đi!', parse_mode='Markdown', reply_markup=keyboard)
            return
        ...
        await asyncio.sleep(1)  # Chờ animation
        ...
        # Kết quả dưới: 3 xúc xắc text một hàng + status (tin mới)
        result_msg = f"""
...
Chơi tiếp?
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await context.bot.send_message(chat_id=query.message.chat_id, text=result_msg, parse_mode='Markdown', reply_markup=reply_markup)
        # Không edit query nữa, để chat flow tự nhiên

    elif query.data in ['score', 'history', 'top', 'help', 'reset', 'menu']:
        # Các phần khác: Gửi tin mới cho mỗi
        if query.data == 'score':
...
• **Xúc xắc:** Bot lăn thật (animation 🎲 x3 trên).
...
🔙 *Menu*
            ...
            keyboard = get_main_keyboard()
            message = '🔥 **Menu chính - Sẵn sàng chơi?** 🎰'

        reply_markup = InlineKeyboardMarkup(keyboard)
        await context.bot.send_message(chat_id=query.message.chat_id, text=message, parse_mode='Markdown', reply_markup=reply_markup)
...
def main():
    ...
    print("Bot Tài Xỉu mới tin nhắn flow đang chạy... Ctrl+C dừng.")
